...
dev:
	...
	docker compose -f docker-compose.dev.yml exec -T app ai_agent db upgrade
	@echo ""
...
stage:
	...
	docker compose -f docker-compose.yml exec -T app ai_agent db upgrade
...
prod:
	...
	@sleep 5
	docker compose --env-file backend/.env -f docker-compose.prod.yml exec -T app ai_agent db upgrade
...
# === Celery ===
celery-worker:
	uv run --directory backend ai_agent celery worker

celery-beat:
	uv run --directory backend ai_agent celery beat

celery-flower:
	uv run --directory backend ai_agent celery flower
	@echo ""
